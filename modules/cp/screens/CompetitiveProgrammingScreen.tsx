...
export default function CompetitiveProgrammingScreen() {
  ...
  return (
    <ThemedView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        <ThemedText type="title" style={styles.title}>算法竞赛</ThemedText>
        <ThemedText style={[styles.subtitle, { color: palette.icon }]}>提交记录、Rating 曲线与比赛追踪</ThemedText>

        <View style={[styles.heroCard, { backgroundColor: palette.card, borderColor: palette.border }]}> 
          <View style={styles.heroTopRow}>
            <ThemedText type="defaultSemiBold" numberOfLines={1} style={styles.heroHandleText}>
              {cfUser?.handle || cfHandle || '未绑定 Handle'}
            </ThemedText>
          ...
          <View style={[styles.globalRankBadge, { backgroundColor: withAlpha(globalRank.color, 0.14), borderColor: withAlpha(globalRank.color, 0.5) }]}>
            <ThemedText style={[styles.globalRankText, { color: globalRank.color }]}>全局段位：{globalRank.name}</ThemedText>
          </View>
          ...
          <View style={styles.metricCompactRow}>
            <View style={[styles.metricCompactItem, { borderColor: palette.border, backgroundColor: palette.background }]}>
              <ThemedText style={[styles.metricCompactLabel, { color: palette.icon }]}>当前</ThemedText>
              <ThemedText type="defaultSemiBold" style={styles.metricCompactValue}>{currentRating ?? '-'}</ThemedText>
            </View>
            <View style={[styles.metricCompactItem, { borderColor: palette.border, backgroundColor: palette.background }]}>
              <ThemedText style={[styles.metricCompactLabel, { color: palette.icon }]}>最高</ThemedText>
              <ThemedText type="defaultSemiBold" style={styles.metricCompactValue}>{maxRating ?? '-'}</ThemedText>
            </View>
            <View style={[styles.metricCompactItem, { borderColor: palette.border, backgroundColor: palette.background }]}>
              <ThemedText style={[styles.metricCompactLabel, { color: palette.icon }]}>AC/提交</ThemedText>
        ...
        <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}> 
          <ThemedText type="subtitle" style={styles.cardTitle}>全局做题反馈（提交后自动加点）</ThemedText>

          <TextInput
            value={feedbackSource}
            onChangeText={setFeedbackSource}
            placeholder="题目平台（选填，Codeforces / 洛谷 / AtCoder / 牛客...）"
          ...
          <TextInput
            ...
            placeholder="知识点/算法点（合并输入，逗号分隔，例如：线段树, 区间dp, 二分）"
            placeholderTextColor={palette.icon}
            style={[styles.feedbackInput, { borderColor: palette.border, color: palette.text, backgroundColor: palette.background }]}
          />

          <ThemedText style={{ color: palette.icon, fontSize: 12 }}>题型由 LLM 自动判断：老题走训练数据快速判定，新题结合题解判定。</ThemedText>
          ...
          <View style={styles.feedbackToggleRow}>
            ...
            >
              <ThemedText style={{ fontSize: 12 }}>{letLLMInferAlgorithms ? '知识点：LLM 自动判断' : '知识点：手动填写'}</ThemedText>
            </TouchableOpacity>
            <TouchableOpacity
              style={[
                styles.diffBtn,
          ...
          {!letLLMInferAlgorithms ? (
            <ThemedText style={{ color: palette.icon, fontSize: 12 }}>已关闭自动判断：请在上方填写知识点/算法点。</ThemedText>
          ) : null}

          {!letLLMInferDifficulty ? (
            <>
              <ThemedText style={{ color: palette.icon, fontSize: 12 }}>Problem Rating（自定义难度档）</ThemedText>
          ...
          {!letLLMInferDifficulty && feedbackDifficulty === 'water' ? (
            <ThemedText style={{ color: palette.icon, fontSize: 12 }}>已选水题：本次可不填写题解。</ThemedText>
          ) : null}

          <TextInput
            value={feedbackSolution}
            onChangeText={setFeedbackSolution}
            placeholder="题解要点（选填；新题建议填写，水题可留空）"
            placeholderTextColor={palette.icon}
            multiline
          ...
          >
            <ThemedText style={{ color: '#fff', fontWeight: '700' }}>{isSubmittingFeedback ? '提交中...' : '提交反馈并加点'}</ThemedText>
          </TouchableOpacity>
        ...
        <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}> 
          <ThemedText type="subtitle" style={styles.cardTitle}>知识体系（模块 + 小知识点）</ThemedText>
          <ThemedText style={{ color: palette.icon, fontSize: 12, marginBottom: 8 }}>
            大模块已固定为统一口径（实现与模拟 / 数据结构 / 动态规划 / 图论 / 数学 / 字符串 / 贪心与构造 / 搜索与枚举 / 博弈与交互）。
          </ThemedText>
          ...
          <View style={styles.knowledgeAddRow}>
            ...
            >
              <ThemedText style={{ color: '#fff', fontWeight: '700' }}>新增子点</ThemedText>
            </TouchableOpacity>
          </View>
                ...
                <View key={`module-${module.id}`} style={[styles.knowledgeItem, { borderColor: palette.border }]}> 
                  ...
                  <View style={styles.knowledgeProgressMeta}>
                    <ThemedText style={{ color: palette.icon }}>
                      模块 Lv.{module.level} · {module.exp}/{moduleNeed}
                    </ThemedText>
                    ...
                    {moduleSubs.map((sub) => {
                      ...
                      return (
                        <View key={`sub-${sub.id}`} style={[styles.knowledgeSubGaugeItem, { borderColor: palette.border, backgroundColor: palette.background }]}> 
                          ...
                          <ThemedText style={{ color: palette.icon, fontSize: 11 }}>Lv.{sub.level}</ThemedText>
                        </View>
                      );
                    })}
                  </View>
        ...
        <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}> 
          <ThemedText type="subtitle" style={styles.cardTitle}>最近分析与加点理由</ThemedText>
            ...
            feedbackRecords.slice(0, 8).map((item) => {
              ...
              return (
                <View key={`reason-${item.id}`} style={[styles.reasonItem, { borderColor: palette.border, backgroundColor: palette.background }]}> 
                  ...
                  <ThemedText style={{ color: palette.icon, fontSize: 12 }}>
                    类型：{item.problemType === 'new' ? '新题' : '老题'}{item.source ? ` · 来源：${item.source}` : ''}
                  </ThemedText>

                  <ThemedText style={[styles.reasonText, { color: palette.text }]}>分析理由：{item.analysisReason || '按题目元信息与题解输入进行题型/难度判断。'}</ThemedText>
                  <ThemedText style={[styles.reasonText, { color: palette.text }]}>加点理由：{item.pointReason || '按难度档与知识点等级衰减计算加点。'}</ThemedText>
                </View>
              );
            })
          ) : (
            <ThemedText style={[styles.emptyText, { color: palette.icon }]}>暂无分析记录，先提交一条反馈即可查看理由</ThemedText>
          )}
        </View>
        ...
        {!cfHandle ? (
          <View style={[styles.errorBox, { backgroundColor: palette.card, borderColor: palette.border }]}> 
            <ThemedText style={{ color: palette.text }}>
              未设置 Codeforces Handle，请到个人中心填写后返回查看数据。
            </ThemedText>
          </View>
        ) : null}
        ...
        {loading ? (
          <View style={styles.loadingWrap}>
            ...
            <ThemedText style={{ color: palette.icon }}>正在同步 Codeforces 数据...</ThemedText>
          </View>
        ) : null}
        ...
        <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}> 
          <ThemedText type="subtitle" style={styles.cardTitle}>Rating 趋势（近 30 场）</ThemedText>
          {ratingLineData.length > 0 ? (
            ...
            <ThemedText style={[styles.emptyText, { color: palette.icon }]}>暂无 Rating 曲线数据</ThemedText>
          )}
        </View>

        <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}> 
          <ThemedText type="subtitle" style={styles.cardTitle}>全局活跃热力图（近 12 周）</ThemedText>
          <ThemedText style={{ color: palette.icon, marginBottom: 6 }}>
            {contributionHeatmap.startLabel} ~ {contributionHeatmap.endLabel} · 总提交 {contributionHeatmap.total}
          </ThemedText>
        ...
        <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}> 
          <ThemedText type="subtitle" style={styles.cardTitle}>全局难度档统计（CF + 手动反馈）</ThemedText>
          {globalDifficultyStats.length > 0 ? (
            ...
            <ThemedText style={[styles.emptyText, { color: palette.icon }]}>暂无可统计的全局难度档数据</ThemedText>
          )}
        </View>

        <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}> 
          <ThemedText type="subtitle" style={styles.cardTitle}>全局标签统计（CF + 手动反馈，已对齐模块）</ThemedText>
          {topTagStats.length > 0 ? (
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} nestedScrollEnabled>
                <View style={[styles.tagCalloutBoard, { width: tagBoardWidth }]}> 
                  ...
                  >
                    ...
                    <ThemedText style={[styles.tagCenterSubText, { color: palette.icon }]}>Solved</ThemedText>
                  </View>
            ...
            <ThemedText style={[styles.emptyText, { color: palette.icon }]}>暂无可统计的 Tag 数据</ThemedText>
          )}
        </View>

        <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}> 
          <ThemedText type="subtitle" style={styles.cardTitle}>最近参与比赛（解题数 / 总题数）</ThemedText>
          {recentContests.length > 0 ? (
            recentContests.map((contest) => (
              <View key={`${contest.contestName}-${contest.ratingUpdateTimeSeconds}`} style={[styles.contestRow, { borderColor: palette.border }]}> 
                <View style={styles.contestMain}>
                  ...
                  <ThemedText style={{ color: palette.icon }}>
                    {new Date(contest.ratingUpdateTimeSeconds * 1000).toLocaleDateString('zh-CN')}
                  </ThemedText>
                  <ThemedText style={{ color: palette.icon }}>
                    题数：{contestSolveMap[contest.contestId]?.solved ?? 0} /
                    {' '}
                    {contestSolveMap[contest.contestId]?.total ?? '?'}
                    {' '}（尝试 {contestSolveMap[contest.contestId]?.attempted ?? 0} 题）
                  </ThemedText>
                </View>
            ...
            <ThemedText style={[styles.emptyText, { color: palette.icon }]}>暂无比赛记录</ThemedText>
          )}
        </View>

        <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}> 
          <View style={styles.upcomingHeaderRow}>
            <ThemedText type="subtitle" style={styles.cardTitle}>即将开始的比赛</ThemedText>
            <TouchableOpacity
              style={[
            ...
            >
              <ThemedText style={[styles.upcomingAddBtnText, { color: selectedUpcomingCount > 0 && !isAddingUpcoming ? '#fff' : palette.icon }]}>
                {isAddingUpcoming ? '添加中...' : `添加到日程(${selectedUpcomingCount})`}
              </ThemedText>
            </TouchableOpacity>
          </View>
          {upcomingActionText ? (
            ...
            upcomingContests.map((contest) => (
              <View key={`upcoming-${contest.id}`} style={[styles.contestRow, { borderColor: palette.border }]}> 
                <View style={styles.contestMain}>
                  ...
                  <ThemedText style={{ color: palette.icon }}>
                    开始：{contest.startTimeSeconds ? new Date(contest.startTimeSeconds * 1000).toLocaleString('zh-CN') : '-'}
                  </ThemedText>
                  <ThemedText style={{ color: palette.icon }}>
                    时长：{formatDuration(contest.durationSeconds)} · {formatRemaining(contest.startTimeSeconds)}
                  </ThemedText>
                </View>
                <TouchableOpacity
                  style={[
                ...
                >
                  <ThemedText style={{ color: selectedUpcomingIds[contest.id] ? palette.tint : palette.icon, fontSize: 12, fontWeight: '600' }}>
                    {selectedUpcomingIds[contest.id] ? '已选' : '勾选'}
                  </ThemedText>
                </TouchableOpacity>
              </View>
            ))
          ) : (
            <ThemedText style={[styles.emptyText, { color: palette.icon }]}>暂无即将开始的比赛</ThemedText>
          )}
        </View>
